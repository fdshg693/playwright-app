...
**現状 `01-vertical-slice.md` だけが詳細まで埋まっており（実装・実行済み）、`02`〜`08` はプレースホルダ（「詳細は後に記載」)。** 新しいステップに着手する際はまず対応する `big_plans/0N-*.md` を読んで完了条件を掴んでから、`.claude/plan/main/0N-*.md` を詳細プランで埋める。
